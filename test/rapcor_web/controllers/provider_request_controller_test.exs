defmodule RapcorWeb.ProviderRequestControllerTest do
  ...
  @create_attrs %{contact_email: "some@email.com", contact_phone: "some contact_phone", end_date: ~D[2010-04-17], notes: "some notes", required_experiences: [%{experience_id: 1, minimum_years: 3}], start_date: ~D[2010-04-17]}
  @update_attrs %{contact_email: "updated@email.com", contact_phone: "some updated contact_phone", end_date: ~D[2011-05-18], notes: "some updated notes", required_experiences: [], start_date: ~D[2011-05-18]}
  @invalid_attrs %{contact_email: "test", contact_phone: nil, notes: nil, required_experiences: nil}
  ...
  describe "index" do
    ...
    test "lists all requests", %{conn: conn, request: request, provider_token: token} do
      ...
      assert json_response(conn, 200)["requests"] == [%{
        ...
        "id" => request.id,
        "notes" => "some notes",
        "required_experiences" => [%{"experience_id" => 1, "minimum_years" => 3}],
  ...
  describe "update request" do
    ...
    test "renders request when data is valid", %{conn: conn, request: %Request{id: id} = request, provider_token: token} do
  ...
  defp create_request(context) do
    ...
    {:ok, request: request, provider: provider, provider_token: token}
  end
