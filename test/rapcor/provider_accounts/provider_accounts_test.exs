  ...
  describe "locations" do
    ...
    def location_fixture(attrs \\ %{}) do
      {:ok, location} =
        attrs
        |> Enum.into(@valid_attrs)
        |> ProviderAccounts.create_location()
    ...
    test "list_locations/0 returns all locations" do
      ...
      assert ProviderAccounts.list_locations() == [location]
    end

    test "get_location!/1 returns the location with given id" do
      location = location_fixture()
      assert ProviderAccounts.get_location!(location.id) == location
    end

    test "create_location/1 with valid data creates a location" do
      assert {:ok, %Location{} = location} = ProviderAccounts.create_location(@valid_attrs)
      assert location.name == "some name"
    end

    test "create_location/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = ProviderAccounts.create_location(@invalid_attrs)
    end

    test "update_location/2 with valid data updates the location" do
      location = location_fixture()
      assert {:ok, location} = ProviderAccounts.update_location(location, @update_attrs)
    ...
    test "update_location/2 with invalid data returns error changeset" do
      location = location_fixture()
      assert {:error, %Ecto.Changeset{}} = ProviderAccounts.update_location(location, @invalid_attrs)
      assert location == ProviderAccounts.get_location!(location.id)
    end

    test "delete_location/1 deletes the location" do
      location = location_fixture()
      assert {:ok, %Location{}} = ProviderAccounts.delete_location(location)
      assert_raise Ecto.NoResultsError, fn -> ProviderAccounts.get_location!(location.id) end
    end

    test "change_location/1 returns a location changeset" do
      location = location_fixture()
      assert %Ecto.Changeset{} = ProviderAccounts.change_location(location)
    end
  end
end
